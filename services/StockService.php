...
class StockService extends BaseService
{
	...
	public function ConsumeProduct(int $productId, float $amount, bool $spoiled, $transactionType, $specificStockEntryId = 'default', $recipeId = null, $locationId = null, &$transactionId = null, $allowSubproductSubstitution = false, $consumeExactAmount = false, $isQuickConsumption = false)
	{
		if (!$this->ProductExists($productId)) {
			throw new \Exception('Product does not exist or is inactive');
		}
		...
		if ($transactionType === self::TRANSACTION_TYPE_CONSUME || $transactionType === self::TRANSACTION_TYPE_INVENTORY_CORRECTION) {
			if ($locationId === null) {
				...
				$potentialStockEntries = $this->GetProductStockEntries($productId, false, $allowSubproductSubstitution, ordered: true, isQuickConsumption: $isQuickConsumption);
			} else {
	...
	public function GetProductStockEntries($productId, $excludeOpened = false, $allowSubproductSubstitution = false, $ordered = true, $isQuickConsumption = false)
	{
		...
		if ($isQuickConsumption) {
			$sqlWhereProductId = $sqlWhereProductId . ' AND (location_id NOT IN (SELECT id FROM locations WHERE block_from_quick_consumption = 1))';
		}
	...
	public function OpenProduct(int $productId, float $amount, $specificStockEntryId = 'default', &$transactionId = null, $allowSubproductSubstitution = false, $isQuickConsumption = false)
	{
		if (!$this->ProductExists($productId)) {
			throw new \Exception('Product does not exist or is inactive');
		}
		...
		$potentialStockEntries = $this->GetProductStockEntries($productId, true, $allowSubproductSubstitution, $isQuickConsumption);
		$product = $this->getDatabase()->products($productId);
